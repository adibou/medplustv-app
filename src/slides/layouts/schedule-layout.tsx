...
// Miroir de medplustv-bo/src/slides/components/layouts/schedule-layout.tsx.
// Compat legacy (row1Hours..row3Hours) idem back : le raw `content` peut encore
// contenir les anciennes clés si la migration n'a pas encore tourné.
export default function ScheduleLayout({ content }: Props) {
    const color = useSlideTextColor();
    const rows = [
        { label: 'Lundi', value: getText(content, 'mondayHours') || getText(content, 'row1Hours') },
        { label: 'Mardi', value: getText(content, 'tuesdayHours') || getText(content, 'row2Hours') },
        { label: 'Mercredi', value: getText(content, 'wednesdayHours') || getText(content, 'row3Hours') },
        { label: 'Jeudi', value: getText(content, 'thursdayHours') },
        { label: 'Vendredi', value: getText(content, 'fridayHours') },
